...
const SITE_SOURCE = 'avocats.arintegration.fr (vertical avocats)'
...
const OFFRES = [
  ...
  { value: 'groupe', label: 'Devis personnalisé (cabinet de groupe / SCP / SELARL)' },
...
function buildFormattedMessage(d: FormData): string {
  ...
  push('Spécialité', 'specialite')
  push("Nombre d'avocats", 'praticiens')
  ...
  return (
    <form onSubmit={onSubmit} className="card-bordered space-y-5 p-6 md:p-8">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <div>
          <label htmlFor="praticiens" className="label-field">Nombre d'avocats</label>
