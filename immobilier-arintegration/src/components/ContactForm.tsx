...
const SITE_SOURCE = 'immobilier.arintegration.fr (vertical immobilier)'
...
const SPECIALITES = [
  'Agence indépendante',
  'Réseau / franchise (Century 21, Orpi, Laforêt, etc.)',
...
const OFFRES = [
  ...
  { value: 'connecte', label: 'Agence Connectée (1050€ lancement)' },
  { value: 'premium', label: 'Agence Premium (1750€ lancement)' },
  { value: 'groupe', label: "Devis personnalisé (réseau d'agences / portefeuille luxe)" },
...
function buildFormattedMessage(d: FormData, fullPhone: string): string {
  ...
  push("Type d'activité", String(d.get('specialite') || '').trim())
  push('Nombre de négociateurs', String(d.get('praticiens') || '').trim())
...
export default function ContactForm() {
  ...
  return (
    <form onSubmit={onSubmit} className="card-bordered space-y-5 p-6 md:p-8">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div>
          <label htmlFor="specialite" className="label-field">Type d'activité</label>
        ...
        <div>
          <label htmlFor="praticiens" className="label-field">Nombre de négociateurs</label>
